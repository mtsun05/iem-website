...
export default function CarsPage() {
  ...
  const isIntersecting = useIsVisible(carRef, 0.3, false);

  const isDefaultView = activePart === null || activePart === "GENERAL";
  const highlightedPart3D =
    activePart && activePart !== "GENERAL" ? activePart : null;
  const isAutoRotating = isDefaultView && isIntersecting;
    ...
    <main className="bg-neutral-900 text-white">
      <div className="h-[calc(100vh-10rem)] w-screen bg-neutral-900 text-white overflow-hidden relative font-sans">
        <div ref={carRef} className="absolute inset-0 z-0">
          <CarShowcase
            highlightedPart={highlightedPart3D}
        ...
        <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between p-8">
          ...
          <div className="flex-1 flex items-center justify-end px-20">
            ...
            >
              {displayInfo && (
                <>
                  <span className="text-transparent bg-linear-to-r from-[#2c5191] to-[#FA6300] bg-clip-text text-3xl font-semibold uppercase mb-4">
                    {displayInfo.title}
                  </span>
                  <h3 className="text-lg text-neutral-300 mb-6">
          ...
          <div className="flex justify-center gap-4 pointer-events-auto mb-8">
            {(
              ["FRONT_SPOILER", "VENTURI", "DIFFUSER", "REAR_WING"] as PartKey[]
            ).map((part) => (
              <button
                key={part}
                className={`px-6 py-4 rounded-xl backdrop-blur-md border min-w-[140px] text-left 
                                        ...
                                        ${
                                          activePart === part
                                            ? "bg-linear-to-r from-[#2c5191] border-none to-[#FA6300] text-black shadow-lg shadow-[#FA6300]-400/20"
      ...
      <section className="w-full py-20 sm:py-32 bg-neutral-950/50">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <div className="mb-12 md:mb-16 text-center">
            <span className="text-white text-7xl font-semibold">Archive</span>
          </div>
          ...
          >
                ...
                <CarouselItem key={index}>
                  <div className="bg-black/60 backdrop-blur-lg p-8 rounded-4xl border border-white/10 h-[600px] flex flex-col mx-2">
                    <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                      ...
                      <div className="w-full md:w-1/2">
                        <span className="text-transparent bg-linear-to-r from-[#2c5191] to-[#FA6300] bg-clip-text text-3xl font-semibold uppercase">
                          {card.name}
                        </span>
            ...
            <CarouselPrevious className="cursor-pointer bg-none text-black border-none hover:bg-neutral-400 scale-150 transition duration-300" />
            <CarouselNext className="cursor-pointer bg-none text-black border-none hover:bg-neutral-400 scale-150 transition duration-300" />
          </Carousel>
        </div>
      </section>
    </main>
