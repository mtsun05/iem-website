...
export function CarModel({ highlightedPart, isAutoRotating, ...props }: CarModelProps) {
    ...
    const diffuserProps = useMemo(() => getMaterialProps('DIFFUSER'), [highlightedPart]);
    const spoilerProps = useMemo(() => getMaterialProps('FRONT_SPOILER'), [highlightedPart]);
    const wingProps = useMemo(() => getMaterialProps('REAR_WING'), [highlightedPart]);
    const venturiProps = useMemo(() => getMaterialProps('VENTURI'), [highlightedPart]);
        ...
        <group ref={rotationRef} {...props}>
            ...
            >
                <Select enabled={highlightedPart === 'DIFFUSER'}>
                    <mesh ref={diffuserRef} geometry={(nodes.DIFFUSER as THREE.Mesh).geometry}>
                        <meshStandardMaterial ref={diffuserMaterialRef} {...darkMaterialProps} />
                    </mesh>
                </Select>

                <Select enabled={highlightedPart === 'FRONT_SPOILER'}>
                    <mesh ref={spoilerRef} geometry={(nodes.FRONT_SPOILER as THREE.Mesh).geometry}>
                        <meshStandardMaterial ref={spoilerMaterialRef} {...darkMaterialProps} />
                    </mesh>
                </Select>

                <Select enabled={highlightedPart === 'REAR_WING'}>
                    <mesh ref={wingRef} geometry={(nodes.REAR_WING as THREE.Mesh).geometry}>
                        <meshStandardMaterial ref={wingMaterialRef} {...darkMaterialProps} />
                    </mesh>
                </Select>

                <Select enabled={highlightedPart === 'VENTURI'}>
                    <mesh ref={venturiRef} geometry={(nodes.VENTURI as THREE.Mesh).geometry}>
                        <meshStandardMaterial ref={venturiMaterialRef} {...darkMaterialProps} />
                    </mesh>
                </Select>

                <primitive ref={backgroundRef} object={nodes.CH_25_FULL_CAR_ASM} rotation={[0, 0, 0]} />
            </group>
        </group>
    );
}
